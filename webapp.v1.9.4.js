// HistorIA v1.9.4 — baseado em v1.9.3 com: 3 opções, "novas opções", gêneros ampliados, narrador fixo com sliders, 1ª pessoa, GitHub sync.
(function(){
  ...
  let scrMenu,scrCreate,scrRead,btnCreate,btnHome,btnSettings,list,genStatus,
      f_title,f_genre,f_nucleos,f_caps,f_brief,f_nuclei,f_firstperson,
      capitulo,titulo,narrativa,choices,btnNext,btnBack,btnSpeak,btnStop,selVoice,
      rateSlider,volSlider,rateVal,volVal,muteBtn,speechStatus,pillRate,pillVol,wrapRate,wrapVol,
      licenseModal,licenseKey,modelKey,gh_owner,gh_repo,gh_branch,gh_token;

  // --- settings / storage
  const LS_SETTINGS='HIA_settings_v194', LS_STORIES='HIA_stories', LS_PROGRESS='HIA_progress';
  let Settings=JSON.parse(localStorage.getItem(LS_SETTINGS)||'{}');
  Settings=Object.assign({license:'',model:'gemini-1.5-pro',rate:1.0,vol:0.9,gender:'auto',muted:false,gh:{}},Settings);
  const saveSettings=()=>localStorage.setItem(LS_SETTINGS,JSON.stringify(Settings));
  const needLicense=()=>!Settings.license||!/^AIza/.test((Settings.license||'').trim()); // Google AI Studio
  ...
  function collect(){
    ...
    f_title=$('f_title'); f_genre=$('f_genre'); f_nucleos=$('f_nucleos'); f_caps=$('f_capitulos'); f_brief=$('f_brief'); f_nuclei=$('f_nuclei'); f_firstperson=$('f_firstperson');
    capitulo=$('capitulo'); titulo=$('titulo'); narrativa=$('narrativa'); choices=$('choices'); btnNext=$('btnNext'); btnBack=$('btnBack');
    btnSpeak=$('btnSpeak'); btnStop=$('btnStop'); selVoice=$('selVoice'); speechStatus=$('speechStatus');
    rateSlider=$('rateSlider'); volSlider=$('volSlider'); rateVal=$('rateVal'); volVal=$('volVal'); muteBtn=$('muteBtn');
    pillRate=$('pillRate'); pillVol=$('pillVol'); wrapRate=$('wrapRate'); wrapVol=$('wrapVol');
    licenseModal=$('license'); licenseKey=$('licenseKey'); modelKey=$('modelKey');
    gh_owner=$('gh_owner'); gh_repo=$('gh_repo'); gh_branch=$('gh_branch'); gh_token=$('gh_token');
  }
  function show(which){
  ...
  function bind(){
    btnSettings.onclick=()=>{ licenseKey.value=Settings.license||''; modelKey.value=Settings.model||'gemini-1.5-pro';
      gh_owner.value=Settings.gh?.owner||''; gh_repo.value=Settings.gh?.repo||''; gh_branch.value=Settings.gh?.branch||'main'; gh_token.value=Settings.gh?.token||'';
      licenseModal.classList.add('on'); };
    $('lic_cancel').onclick=()=>licenseModal.classList.remove('on');
    $('lic_ok').onclick=()=>{ Settings.license=licenseKey.value.trim(); Settings.model=(modelKey.value||'gemini-1.5-pro').trim();
      Settings.gh={owner:gh_owner.value.trim(),repo:gh_repo.value.trim(),branch:(gh_branch.value||'main').trim(),token:gh_token.value.trim()};
      saveSettings(); licenseModal.classList.remove('on'); };
    ...
    rateSlider.addEventListener('input',()=>{Settings.rate=parseFloat(rateSlider.value)||1.0; rateVal.textContent=Settings.rate.toFixed(2)+'x'; saveSettings(); restartIfSpeaking();});
    volSlider.addEventListener('input',()=>{Settings.vol=parseFloat(volSlider.value)||0.9; volVal.textContent=Math.round(Settings.vol*100)+'%'; saveSettings(); restartIfSpeaking();});
    muteBtn.onclick=()=>{Settings.muted=!Settings.muted; saveSettings(); restartIfSpeaking();};

    // safety rebind
    setInterval(()=>{ if(!btnCreate.onclick){ bind(); } }, 2000);
  }
  ...
  function clearCreator(){ f_title.value=''; f_brief.value=''; f_nuclei.value=''; f_genre.value='drama'; f_nucleos.value=6; f_caps.value=10; f_firstperson.checked=false; }

  // --- text helpers
  function antiRepeat(text){ if(!text) return ''; const s=text.replace(/\\s+/g,' ').trim(); const parts=s.split(/(?<=[\\.!?…])\\s+/); const out=[]; const seen=new Map(); for(const p of parts){const k=p.lower? p.lower(): p.lowercase? p.lowercase(): p.lower?? p.toLowerCase(); seen.set(k,(seen.get(k)||0)+1); if(seen.get(k)<=1) out.push(p);} return out.join(' '); }
  ...
  async function callGemini(prompt){
    ...
    const body={{contents:[{{role:"user",parts:[{{text:prompt}}]}}],generationConfig:{{temperature:0.9,topP:0.9,maxOutputTokens:16384}}}};
...
Distribua os núcleos ao longo dos ${meta.total} capítulos (todos devem aparecer). Foque em enredo e diálogos, sem clichês de 'atmosfera densa'.${meta.first?' Estilo em 1ª pessoa, protagonista é o leitor.':''}`;
  }
...
{{ "titulo": string,
  "parte1": string,
  "decisao1": {{ "A": {{"label": string, "texto": string}}, "B": {{"label": string, "texto": string}}, "C": {{"label": string, "texto": string}} }},
  "parte2": string,
  "decisao2": {{ "A": {{"label": string, "texto": string}}, "B": {{"label": string, "texto": string}}, "C": {{"label": string, "texto": string}} }},
  "conclusao": string,
  "state_updates": {{ "fatos": [string], "nucleos_usados": [string] }}
}}
Regras duras: continuidade obrigatória com capítulos anteriores e decisões tomadas. As opções devem ser DIVISORAS do que acabou de ocorrer (nada de ações já superadas). Use TODOS os elementos citados em Enredo/Núcleos ao longo da temporada; priorize os ainda não usados: ${remaining.join(', ')||'N/A'}.
Evite clichês de atmosfera. Foque em ações, detalhes concretos e diálogos.${meta.first?' Narrativa em PRIMEIRA PESSOA.':''}
...
${text}`;
  ...
  async function generateOneChapter(meta,indexZero,existing,story){
    ...
    const mid = obj.decisao1||{}; const end=obj.decisao2||{};
    const pick=(x)=>({label:x?.label||'Opção',text:antiRepeat(antiCliche(x?.texto||''))});
    return { title:obj.titulo||`Capítulo ${indexZero+1}`,
      intro1:antiRepeat(p1),
      mid:{A:pick(mid.A),B:pick(mid.B),C:pick(mid.C)},
      part2:antiRepeat(p2),
      end:{A:pick(end.A),B:pick(end.B),C:pick(end.C)},
      outro:antiRepeat(pc)};
  }

  // --- regenerate options
  async function regenerateOptions(phase){
    if(!current || !current.chapters[idx]) return;
    const meta={title:current.title,genre:current.genre,brief:current.metaBrief,nuclei:current.metaNuclei,total:current.total,first:current.firstperson};
    const ch=current.chapters[idx];
    const context = phase==='50' ? ch.intro1 : (ch.intro1+' '+ch.part2);
    const prompt=`Retorne APENAS JSON válido com exatamente 3 novas opções COERENTES e DIVISORAS do que acabou de ocorrer.
Schema: { "A":{"label":string,"texto":string}, "B":{"label":string,"texto":string}, "C":{"label":string,"texto":string} }
Contexto: ${context}`;
    try{
      const raw=await callGemini(prompt);
      const obj=robustParseJSON(raw)||{};
      const pick=(x)=>({label:x?.label||'Opção',text:antiRepeat(antiCliche(x?.texto||''))});
      if(phase==='50'){ ch.mid={A:pick(obj.A),B:pick(obj.B),C:pick(obj.C)}; }
      else { ch.end={A:pick(obj.A),B:pick(obj.B),C:pick(obj.C)}; }
      persistCurrent(); render();
      toast('Novas opções prontas');
    }catch{ toast('Falha ao gerar novas opções'); }
  }

  // --- state / navigation
  let currentEdit=null, backgroundBusy=false;
  async function onGenerate(){
    if(needLicense()){ licenseModal.classList.add('on'); genStatus.textContent='Informe a licença para gerar.'; return;}
    ...
    const first=f_firstperson.checked;
    if(!brief||!nuclei){genStatus.textContent='Preencha Enredo e Núcleos.'; return;}
    genStatus.textContent='Construindo bíblia...';
    const story={id:currentEdit||Math.random().toString(36).slice(2),title,genre,nucleos:nucleosCount,total,metaBrief:brief,metaNuclei:nuclei,firstperson:first,chapters:[],decisions:[]};
    const meta={title,genre,brief,nuclei,total,first};
    try{
  ...
  function editStory(id){
    ...
    currentEdit=st.id;
    f_title.value=st.title; f_genre.value=st.genre; f_nucleos.value=st.nucleos; f_capitulos.value=st.total; f_brief.value=st.metaBrief||''; f_nuclei.value=st.metaNuclei||''; f_firstperson.checked=!!st.firstperson;
    show('create');
  }

  let current=null,idx=0,step=0,path=[];
  function persistCurrent(){ const arr=getAll(); const i=arr.findIndex(x=>x.id===current.id); if(i>=0){arr[i]=current; setAll(arr);} maybeSync(current); }
  function saveProg(){const p=getProgress(); p[current.id]={idx,step}; setProgress(p); renderMenu(); persistCurrent();}
  ...
  function ensureNextInBackground(){
    ...
    if(next<current.total && !chapterReady(next)){
      ...
      generateOneChapter(meta,next,current.chapters,current).then(ch=>{
        current.chapters[next]=ch; persistCurrent();
      }).finally(()=>backgroundBusy=false);
    }
  }
  function choiceIcon(label){
    const s=(label||'').toLowerCase();
    if(/porta|entrar|sair/.test(s)) return '🚪';
    if(/fug|correr|perigo/.test(s)) return '🏃';
    if(/investig|procur|vasculh|examinar/.test(s)) return '🔍';
    if(/confront|enfrent|discut|falar/.test(s)) return '🗣️';
    if(/esper|aguard|cautel/.test(s)) return '⏳';
    if(/romance|beijo|amor/.test(s)) return '💞';
    return '✨';
  }
  function renderChoices(set, phase){
    choices.innerHTML='';
    const opts=[['A',set.A],['B',set.B],['C',set.C]];
    opts.forEach(([k,o])=>{
      if(!o) return;
      const div=document.createElement('div'); div.className='choice';
      div.innerHTML=`<div class="ico">${{choiceIcon(o.label)}}</div><div style="flex:1"><div class="label">${{k}}) ${{o.label}}</div><div class="mini">${{(o.text||'').slice(0,180)}}…</div></div><div class="btns"><button class="btn" data-k="${{k}}">Escolher</button></div>`;
      choices.appendChild(div);
      el('button[data-k="'+k+'"]',div).onclick=()=>onChoice(k);
    });
    const regen=document.createElement('div'); regen.style.textAlign='right'; regen.innerHTML=`<button id="btnRegen" class="btn">🔄 Novas opções</button>`;
    choices.appendChild(regen);
    $('btnRegen').onclick=()=>regenerateOptions(phase);
  }
  function render(){
    const ch=current.chapters[idx];
    capitulo.textContent='Capítulo '+(idx+1)+' de '+(current.total||current.chapters.length);
    titulo.textContent=ch? ch.title : 'Gerando...';
    if(!ch){ setText('Gerando este capítulo...'); choices.innerHTML=''; btnNext.textContent='Aguarde'; return; }
    if(step===0){ setText(ch.intro1); choices.innerHTML=''; btnNext.textContent='Avançar ⟶'; }
    else if(step===1){ setText('Decisão (50%)'); renderChoices(ch.mid,'50'); btnNext.textContent='Escolha A/B/C'; }
    else if(step===2){ setText(ch.part2); choices.innerHTML=''; btnNext.textContent='Avançar ⟶'; }
    else if(step===3){ setText('Decisão (90%)'); renderChoices(ch.end,'90'); btnNext.textContent='Escolha A/B/C'; }
    else if(step===4){ setText(ch.outro); choices.innerHTML=''; btnNext.textContent=(idx<(current.total-1))?'Próximo capítulo ⟶':'Finalizar'; ensureNextInBackground(); }
    ...
    if(step===0) step=1;
      ...
      if(idx<(current.total-1)){
        ...
        if(!chapterReady(idx)){
          ...
          const meta={title:current.title,genre:current.genre,brief:current.metaBrief,nuclei:current.metaNuclei,total:current.total,first:current.firstperson};
          try{ const ch=await generateOneChapter(meta,idx,current.chapters,current); current.chapters[idx]=ch; persistCurrent(); }catch{ setText('Falha ao gerar. Verifique a licença.'); }
        }
  ...
  function speakFrom(i=0){
    ...
    function stepSpeak(){
      ...
      if(voice) u.voice=voice; u.rate=Settings.rate||1.0; u.volume=(Settings.muted?0:(Settings.vol||0.9));
  ...
  // --- GitHub sync (opcional, silencioso)
  async function maybeSync(story){
    try{
      const gh=Settings.gh||{}; if(!gh.owner||!gh.repo||!gh.branch||!gh.token) return;
      const path=`stories/${story.id}.json`;
      const api=`https://api.github.com/repos/${gh.owner}/${gh.repo}/contents/${path}`;
      const content=btoa(unescape(encodeURIComponent(JSON.stringify(story))));
      // get sha if exists
      let sha=null;
      const get=await fetch(api);
      if(get.ok){ const j=await get.json(); sha=j.sha; }
      const res=await fetch(api,{method:'PUT',headers:{'Content-Type':'application/json','Authorization':`token ${gh.token}`},
        body:JSON.stringify({message:`update ${story.id}`,content:content,branch:gh.branch,sha})});
      // ignore result
    }catch(e){/*silencioso*/}
  }
  ...
  function init(){
    ...
    rateVal.textContent=(Settings.rate||1.0).toFixed(2)+'x'; rateSlider.value=(Settings.rate||1.0).toFixed(2);
    volVal.textContent=Math.round((Settings.vol||0.9)*100)+'%'; volSlider.value=(Settings.vol||0.9).toFixed(2);
    if(needLicense()) licenseModal.classList.add('on');
    toast('HistorIA v1.9.4 pronto');
  }
