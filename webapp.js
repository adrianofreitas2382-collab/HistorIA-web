const APP={version:'v2.1.1',state:{voice:'auto',rate:1,volume:0.9,muted:false,apiKey:localStorage.getItem('geminiKey')||'AIzaSyBrcPS9mYDqnEeseSeb5huoM0kZ0JcszTM',model:localStorage.getItem('geminiModel')||'gemini-1.5-pro',roteiros:[],db:null}};
...
- ~15 min de leitura; cenas e diálogos concretos (evite clichês vagos).
- Núcleos: ${ctx.nucleos}.
- Enredo breve guia metade: ${ctx.enredo}.
- Continue exatamente do ponto: ${ctx.historico||'Início.'}
- Inserir DECISÃO 50% e DECISÃO 90% com 3 opções A/B/C coerentes ao momento.
...
function renderList(){
  const ul=$('#listRoteiros'); ul.innerHTML='';
  APP.state.roteiros.forEach(r=>{
    const li=el('div','item');
    const left=el('div');
    left.appendChild(el('div','item-title',`${r.titulo} — <span class="kv">${r.genero} • ${r.caps} caps</span>`));
    left.appendChild(el('div','kv',`Núcleos: ${r.nucleos}`));
    left.appendChild(el('div','kv',r.statusText||statusHuman(r.status)));
...
function abrirRoteiro(r){
  // placeholder de leitura; você pode evoluir para leitor completo
  alert('Abrindo: '+r.titulo+'\\nCapítulos prontos: '+r.capitulos.length+'/'+r.caps);
}
...
async function gerarCapitulo(r,num){
  ...
  try{
    ...
    const texto=await geminiGenerate(buildChapterPrompt(ctx));
    ...
    r.capitulos.push({num,texto,op50:ops('DECISÃO 50%'),op90:ops('DECISÃO 90%')});
    r.historico+=`\\n[Capítulo ${num} concluído]\\n`;
    r.status='ok'; r.statusText='OK'; await dbPut(r); renderList();
    if(num<r.caps){ setTimeout(()=>gerarCapitulo(r,num+1),500); }
  }catch(e){
    r.status='api:'+e.message; r.statusText=statusHuman(r.status); await dbPut(r); renderList();
  }
}

function wireUI(){
  $('#version').textContent=APP.version;
  $('#btnVel').onclick=()=>$('#wrapVel').classList.toggle('open');
  $('#btnVol').onclick=()=>$('#wrapVol').classList.toggle('open');
  $('#rngVel').oninput=e=>{APP.state.rate=parseFloat(e.target.value); if(utter) utter.rate=APP.state.rate;};
  $('#rngVol').oninput=e=>{APP.state.volume=parseFloat(e.target.value); if(utter) utter.volume=APP.state.volume;};
  $('#btnNarrar').onclick=()=>{const r=APP.state.roteiros[0]; if(!r||!r.capitulos.length){alert('Abra um roteiro.'); return;} ttsSpeak(r.capitulos[0].texto.slice(0,900));};
  $('#btnParar').onclick=()=>ttsStop();
  $('#btnTutorial').onclick=()=>$('#modalTutorial').classList.add('open');
  $('#btnFecharTut').onclick=()=>$('#modalTutorial').classList.remove('open');
  $('#btnIA').onclick=()=>{ $('#inpKey').value=APP.state.apiKey||''; $('#inpModel').value=APP.state.model||'gemini-1.5-pro'; $('#modalIA').classList.add('open'); };
  $('#btnIAFechar').onclick=()=>$('#modalIA').classList.remove('open');
  $('#btnIASalvar').onclick=()=>{ const k=$('#inpKey').value.trim(); const m=$('#inpModel').value.trim(); if(k){APP.state.apiKey=k; localStorage.setItem('geminiKey',k);} if(m){APP.state.model=m; localStorage.setItem('geminiModel',m);} $('#modalIA').classList.remove('open'); };
  ...
  $('#btnCriar').onclick=async()=>{
    $('#novoErr').textContent='';
    const d={
      titulo:$('#inpTitulo').value.trim()||'Sem título',
      genero:$('#selGenero').value,
      nucleos:$('#inpNucleos').value.trim(),
      caps:Math.min(10,Math.max(1,parseInt($('#inpCaps').value||'10'))),
      p1:$('#selP1').value==='Sim',
      enredo:$('#txtEnredo').value.trim()
    };
    const r=blankR(d); APP.state.roteiros.unshift(r); await dbPut(r); renderList();
    $('#modalNovo').classList.remove('open'); document.body.style.overflow='auto';
    gerarCapitulo(r,1);
  };
}
